...
{
    public class HediffWithComps_GVirusLocal : Hediff_Injury
    {
        ...
        /// </summary>
        private HediffWithComps_GVirus gVirusGlobalComp = null;
        public HediffWithComps_GVirus GVirusGlobalComp
        {
            get
            {
                if (gVirusGlobalComp == null)
                {
                    var getAttempt = pawn.health.hediffSet.GetFirstHediffOfDef(HediffDef.Named("RE_GVirus"));
                    if (getAttempt == null)
                    {
                        //HediffGiverUtility.TryApply(pawn, HediffDef.Named("RE_GVirus"), new List<BodyPartDef> { BodyPartDefOf.Body });
                        HealthUtility.AdjustSeverity(pawn, HediffDef.Named("RE_GVirus"), 0.1f);
                        gVirusGlobalComp = (HediffWithComps_GVirus)pawn.health.hediffSet.GetFirstHediffOfDef(HediffDef.Named("RE_GVirus"));
                    }
                    else
                        gVirusGlobalComp = (HediffWithComps_GVirus)getAttempt;
                }
                return gVirusGlobalComp;
            }
        }
        ...
        public override void PostAdd(DamageInfo? dinfo)
        {
            base.PostAdd(dinfo);
            if (GVirusGlobalComp == null)
                Log.Warning($"Unable to add G-Virus to {pawn.Label}");
        }
        ...
        public void Notify_SpreadToNextUninfectedPart()
        {
            ...
            Hediff hediff2 = HediffMaker.MakeHediff(HediffDef.Named("RE_GVirusLocalHediff"), pawn, partRecord);
